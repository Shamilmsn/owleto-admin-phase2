...
<div class="content-header">
  <div class="container-fluid">
    <div class="row mb-2">
      <div class="col-sm-6">
        <h1 class="m-0 text-dark">{{trans('lang.market_plural')}}
{{--          <small class="ml-3 mr-3">|</small><small>{{trans('lang.market_desc')}}</small>--}}
        </h1>
      </div><!-- /.col -->
      <div class="col-sm-6">
...
<div class="content">
  ...
  <div class="card">
    ...
    <div class="card-body">
      {!! Form::model($market, ['route' => ['markets.update', $market->id], 'method' => 'patch', 'id' =>'form-update' ]) !!}
      <div class="row">
        @if($customFields)
            ...
            <div class="checkbox">
              <label class="col-9 ml-2 form-check-inline">
                <div class="form-check">
                  ...
                  <input class="form-check-input" name="closed" type="checkbox" value="1" id="closed_vendor"
                          {{ ($market->closed == 1 ? ' checked' : '') }}>
                </div>
              </label>
            </div>
          </div>
            ...
            <div class="checkbox">
              <label class="col-9 ml-2 form-check-inline">
                <div class="form-check">
                  ...
                  <input class="form-check-input" name="available_for_delivery" type="checkbox" value="1"
                         {{ ($market->available_for_delivery == 1 ? ' checked' : '') }} id="available_for_delivery">
                </div>
              </label>
            </div>
          </div>
        ...
        <!-- Description Field -->
          ...
          <div class="form-group row ">
            ...
            <div class="col-9">
              ...
              <input type="hidden" id="latitude" name="latitude" value="{{$market->latitude}}"/>
              <input type="hidden" id="longitude" name="longitude" value="{{$market->longitude}}"/>
            </div>
          </div>
        </div>
...
<script>
  $(function (){
    ...
    var sector = {!! json_encode($market->primary_sector_id) !!};
    instaContainer(sector);

    var sectors = <?php echo json_encode($fieldsSelected); ?>;
      var values = sectors;
      sectorCategories(values);


    $.each( sectors, function( key, value ) {
      instaContainer(value);
    });
      ...
      if (values) {
        $.ajax({
          ...
          success: function (data) {
            ...
            $.each(data, function (key, value) {
              var categories = @json($categoriesIdsSelected);
              $('#category_id').append('<option value="' + value.id + '"'  +   '>' + value.name+ '</option>');
            });
            $('#category_id').val(@json($categoriesIdsSelected));
      ...
      if (cityId) {
        $.ajax({
          ...
          success: function (data) {
            ...
            $.each(data, function (key, value) {
              $('#area_id').append('<option value="' + value.id + '"' + (value.id == area_id ? 'selected="selected"' : '') +   '>' + value.name+ '</option>');
            });
          }
        });
      }
...
<script>
  $( document ).ready(function() {
    ...
    var lat = {!! json_encode($market->latitude) !!};
    var long = {!! json_encode($market->longitude) !!};
