...
function getDateColumn($modelObject, $attributeName = 'updated_at')
{
    ...
    $updateDate = \Carbon\Carbon::parse($modelObject[$attributeName])->format('d M Y');

    if (setting('is_human_date_format', false)) {
        $html = '<p data-toggle="tooltip" data-placement="bottom" title="${date}">'.$updateDate.'</p>';
    } else {
        $html = '<p data-toggle="tooltip" data-placement="bottom" title="${dateHuman}">'.$updateDate.'</p>';
    }
...
function getPhoneColumn($column, $attributeName)
{
    if (isset($column)) {
        if ($column[$attributeName]) {
            return "<a class='btn btn-outline-secondary btn-sm' href='mailto:" . $column[$attributeName] . "'><i class='fa fa-phone mr-1'></i>" . $column[$attributeName] . "</a>";
        } else {
            return '';
        }
    }
...
function getDriverRequestCount()
{
    $pendingDriverRequest = \App\Models\Driver::query()
        ->where('drivers.city_id', auth()->user()->city_id)
...
function getMerchantRequestCount()
{
    $pendingDriverRequest = \App\Models\MerchantRequest::query()
        ->where('status', \App\Models\MerchantRequest::PENDING_STATUS)
        ->get();
